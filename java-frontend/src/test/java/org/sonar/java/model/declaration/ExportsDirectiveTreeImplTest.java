...
public class ExportsDirectiveTreeImplTest {
  ...
  public void exports_with_modules() {
    ...
    ListTree<TypeTree> moduleNames = exports.moduleNames();
    assertThat(moduleNames).hasSize(2);
    assertThat(((MemberSelectExpressionTree) moduleNames.get(0)).identifier().name()).isEqualTo("module1");
    assertThat(moduleNames.separators()).hasSize(1);
    assertThat(moduleNames.separators().iterator().next().text()).isEqualTo(",");
    assertThat(((IdentifierTree) moduleNames.get(1)).name()).isEqualTo("module2");
  ...
  public void test_BaseTreeVisitor() {
    ...
    assertThat(moduleDeclarationVisitor.identifiers).containsExactly("Bar", "Bar", "greetings", "foo", "bar", "module1", "module2");
  }
